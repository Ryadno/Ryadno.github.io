...
thirdInput = document.querySelector("#first div div:nth-child(5) input");
...
const firstDivDiv = document.querySelectorAll("#first div:nth-child(-n+2) div:not(.error)");
const payType = document.querySelectorAll(".pay_type input");

const tarif_all = document.querySelector("#second > div:nth-child(1)");
const pay_all = document.querySelector("#second_second > div:nth-child(1)");

const allErrorP = document.querySelectorAll(".errorP");
finalBtn.addEventListener('click', ()=>{
	firstDivDiv.forEach((e)=>{
		e.style.border = "1px solid grey";
	});
	pay_all.style.border = "none";
	tarif_all.style.border = "none";
	//payType.forEach((e)=>{ // Очищаем paytype
	//	e.style.outline = 'none';
	//});
	//tarifs.forEach((e)=>{ //Очищаем тарифы
	//	e.style.outline = 'none';
	//});
	allErrorP.forEach((e)=>{ //Очищаем ошибки
		e.innerHTML = "";
	});
	...
	for(let i=0; i<inputData.length; i++){
		...
		if(inputData[i].value == ""){
			...
			allErrorP[i].innerHTML = "Пустое значение";
		}
	...
	if(!emailRegex.test(document.querySelector("#first div:first-child div:nth-child(3) input").value)){
		document.querySelector("#first div:first-child div:nth-child(3)").style.border = "1px solid red";
		allErrorP[1].innerHTML = "Неправильный формат email";
		foundANYError = true;
	}
	if(!phoneRegex.test(thirdInput.value)){
		document.querySelector("#first div div:nth-child(5)").style.border = "1px solid red";
		allErrorP[2].innerHTML = "Неправильный формат телефона";
		foundANYError = true;
	}else if(thirdInput.value.length < 11){
		document.querySelector("#first div div:nth-child(5)").style.border = "1px solid red";
		allErrorP[2].innerHTML = "Слишком короткий телефон";
		foundANYError = true;
	}
	if(deliveryChecked){
		...
		if(!tarifFound){
			...
			tarif_all.style.border = "1px solid red";
		}else{
			tarifs.forEach((e)=>{
				e.style.outline = 'none';
			});
	...
	if(!foundPayType){
		...
		pay_all.style.border = "1px solid red";
	}else{
		payType.forEach((e)=>{
			e.style.outline = 'none';
		});
